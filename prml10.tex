...
\subsection{ウィシャート分布}
$$
\calW(\Lambda,W,\nu)=B(W,\nu)|\Lambda|^{\frac{\nu-D-1}{2}}\exp(-\frac{1}{2}\tr (W^{-1}\Lambda).
$$
$$
...
\begin{eqnarray*}
\log \rho_{nk}
 &=& E[\log \pi_k] + \frac{1}{2}E[\log |\Lambda_k|]-\frac{D}{2}\log (2\pi)-\frac{1}{2}E[\quads{\Lambda_k}{(x_n-\mu_k)}]\\
 &=& \log \tilde{\pi}_k + \frac{1}{2}\log \tilde{\Lambda}_k
       - \frac{D}{2}\log (2\pi)-\frac{1}{2}(D\beta_k^{-1}+\nu_k\quads {W_k}{(x_n-m_k)}).
\end{eqnarray*}
よって
$$
...
$$
r_{nk} \propto \pi_k|\lambda_k|^{1/2} \exp (-\frac{1}{2}\quads{\Lambda_k}{(x_n-\mu_k)}).
$$
これは上の式とよく似ている.
...
\section{変分下限}
$q(Z,\vpi,\mu,\Lambda)=q(Z)q(\vpi,\mu,\Lambda)$と分解できると仮定すると
\begin{eqnarray*}
...
E[\log p(X|Z,\mu,\Lambda)]
 ...
 &=& \half \sum_k N_k (\log \tilde{\Lambda}_k - D \log (2\pi))-
    \half \underbrace{\sum_{n,k} r_{nk} (D\beta_k^{-1}+\nu_k \quads{W_k}{(x_n-m_k)})}_{X \text とおく}.
\end{eqnarray*}
$$
X = \sum_k N_k D\beta_k^{-1} + \sum_k \nu_k (\underbrace{\sum_n r_{nk} \quads{W_k}{(x_n-m_k)}}_{Y \text とおく}).
$$
$$
...
E[\log q(\mu,\Lambda)]
 ...
 &=& \sum_k \half \log \tilde{\Lambda}_k + \frac{D}{2}\log (\frac{\beta_k}{2\pi}) -
  \half \underbrace{\tr E[(\beta_k \Lambda_k)\outp{(\mu_k-m_k)}}_{X \text とおく}] + \underbrace{E[\log W]}_{Y \text とおく}\end{eqnarray*}
\begin{eqnarray*}
...
\begin{eqnarray*}
E[\log p(\mu,\Lambda)]
 &=& \sum_k \underbrace{E[\log \calN(\mu_k|m_0,(\beta_0 \Lambda_k)^{-1})]}_{A \text とおく}
...
\calL
 ...
 &&  - \half \sum_k \nu_k \tr(W_k(\underbrace{N_k S_k+N_k\outp{(\bar{x}_k-m_k)}+\beta_0\outp{(m_k-m_0)}+W_0^{-1}}_{=X \text とおく}))\\
