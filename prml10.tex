...
\end{eqnarray*}
よって$q(m) \propto p(m) \exp \calL_m$のとき$\calL$は最大値をとる.
...
$\KL(q||p)$は一般に0ではない.
その意味で\ref{lower_bound}章で述べたように変分下界が正しいと思われる.
...
B &=& E[\log B(W_0,\nu_0)+\frac{\nu_0-D-1}{2}\log |\Lambda_k|-\half \tr (W_0^{-1}\Lambda_k)]\\
  &=& \log B(W_0,\nu_0)+\frac{\nu_0-D-1}{2} \log \tilde{\Lambda)}k - \half \tr (W_0^{-1}\underbrace{E[\Lambda_k]}_{=\nu_k W_k}).
\end{eqnarray*}
よって
\begin{eqnarray*}
E[\log p(\mu,\Lambda)]
