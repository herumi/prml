...
よって
\begin{eqnarray*}
A &=& \int \calN\left(\mu|\frac{x+\beta m}{\beta + 1}, \left((\beta+1)\Lambda\right)^{-1}\right)\frac{1}{(2\pi)^{D/2}}
      \frac{|\beta \Lambda^2|^{1/2}}{|(\beta+1)\Lambda|^{1/2}}\exp\left(-\half \quads{\left(\frac{\beta}{\beta+1}\Lambda\right)}{(x-m)}\right)\,d\mu\\
  &=& \calN\left(x|m,(\frac{\beta}{\beta+1}\Lambda)^{-1}\right).
\end{eqnarray*}
つまり
$$
Y = \calN\left(\hat{x}|m_k,\left(\frac{\beta_k}{\beta_k+1}\Lambda_k\right)^{-1}\right).
$$
\begin{eqnarray*}
D
 &:=& \calN\left(x|m,\left(\frac{\beta}{\beta+1}\Lambda\right)^{-1}\right)W(\Lambda|W,\nu)\\
