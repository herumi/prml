...
$$
H(f)=(\frac{\partial^2}{\partial x_i x_j}f(x))
$$
をヘッセ行列という.
...
比例係数は
$$
\diff{\beta} \lambda_i = \frac{\lambda_i}{\beta}.
$$
よって
$$
...
次に$\alpha$, $\beta$を求める方法を考える.
$$
p(\alpha,\beta|\bm{t}) \propto p(\bm{t}|\alpha,\beta)p(\alpha,\beta)
$$
である. ここで$p(\alpha,\beta)$はほぼ平坦, つまり$\alpha$, $\beta$の値によって変動しないという仮定を置く.
...
\begin{figure}[ht]
 \begin{minipage}{1\hsize}
  \begin{center}
   \includegraphics[bb=0 0 300 250]{para-dep.pdf}
  \end{center}
