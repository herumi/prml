...
\begin{eqnarray*}
\int p(x) S(\log \frac{p(x)}{q(x)})\,dx &=& \int p(x) (\frac{q(x)}{p(x)} + \log \frac{p(x)}{q(x)} - 1)\,dx\\
 &=& \KL(p||q) + \int (q(x) - p(x))\,dx\\
 &=& \KL(p||q).
\end{eqnarray*}
...
E(w) &=& \frac{\beta}{2}||t-\Phi w||^2+\frac{\alpha}{2}\trans{w}w\\
     &=& \half\trans{w}(\alpha I + \beta \trans{\Phi}\Phi)w-\beta \trans{t}\Phi w+\frac{\beta}{2}||t||^2\\
...
\begin{eqnarray*}
E(w) &=& \int \exp(-E(w))\,dw \\
     &=& \exp(-E(m_N)) \int \exp(-\half\trans{(w-m_N)}A{(w-m_N)})\,dw\\
...
$$
\diff{\alpha} \log p(\bm{t}|\alpha,\beta)=\frac{M}{2\alpha}-\half\trans{m_N}m_N - \half\sum \frac{1}{\lambda_i + \alpha}=0.
$$
よって
$$
