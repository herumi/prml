...
(S_W+\frac{N_1 N_2}{N}S_B)w&=&N(\bmm_1-\bmm_2)\\
                           &+&\{N\bmm \trans{\bmm}-N_1\bmm_1 \trans{\bmm_1}-N_2 \bmm_2 \trans{\bmm_2}+\frac{N_1 N_2}{N} (\bmm_1 -\bmm_2)\trans{(\bmm_1-\bmm_2)}\}w.
\end{eqnarray*}
...
\begin{eqnarray*}
 && -\half\sum_{n=1}^N(t_n \log |A|+t_n \trans{(x_n-\mu_1)}A^{-1}(x_n-\mu_1)\\
 &&+(1-t_n) \log |A|+(1-t_n)\trans{(x_n-\mu_2)}A^{-1}(x_n-\mu_2))\\
...
\log p(D)&=&\log Z \approx \log f(\theta_{\text{MAP}}) + \log \sqrt{\frac{(2\pi)^M}{|A|}}\\
         &=&\log p(D|\theta_{\text{MAP}})+(\log p(\theta_{\text{MAP}})+\frac{M}{2} \log (2\pi) - \half \log |A|).
\end{eqnarray*}
括弧内の後ろ三項をOccam係数という.
