...
$$
ここで
$\dif{a_{ij}} a_{st} = \delta_{is} \delta_{jt}$を使った. つまり添え字$s$, $t$が走るときに, $s=i$, $t=j$のときのみが生き残るというわけである.
...
を最大化してみよう.
$$
\dif{w_0}E=\sum(\trans{w}x_n+w_0-t_0)=0
$$
...
よって
\begin{eqnarray*}
\dif{\mu}L &=& \frac{\lambda}{2\pi} \exp\left(-\half\left(\frac{\lambda^2\mu^2}{1+\lambda^2 \sigma^2}\right)\right) I\\
