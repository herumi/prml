...
この形の関数を使うモデルをロジスティック回帰（logistic regression）という.
...
$x_n$が与えられたときにより近い値$x_{n+1}$を見つける. $x_n$における接線の方程式
$$
f'(x_n)(x-x_n)+f(x_n)=0
$$
の解を$x_{n+1}$とすると
...
ラプラス近似をするために$w_{\text MAP}$を平均とするガウス分布で近似しよう. $w_{\text MAP}=m_N$と置く.
共分散は今まで何度もやった通り
$$
...
$$
デルタ関数の性質から$a=\trans{w}\phi$とおくと
$$
\sigma(a)=\sigma(\trans{w}\phi)=\int \delta(x-\trans{w}\phi)\sigma(x)\,dx.
$$
よって
\begin{eqnarray*}
p(C_1|\phi,t) &\approx& \int \Bigl(\int \delta(x-\trans{w}\phi)\sigma(x)q(w)\,dx\Bigr) dw\\
 &=&\int \Bigl(\int \delta(x-\trans{w}\phi)q(w)\,dw\Bigr) \sigma(x)\,dx\\
 &=&\int p(x)\sigma(x)\,dx.
\end{eqnarray*}
ただし
$$
p(x)=\int \delta(x-\trans{w}\phi)q(w)\,dw
$$
とおいた.

$p(x)$の平均を求める. $q(w)$の平均が$m_N$であることに注意すると,
\begin{eqnarray*}
\mu_a &=& E[a]=\int p(a)a\,da=\int \delta(a-\trans{w}\phi)q(w)a\,dw da=\int q(w)(\trans{w}\phi)\,dw\\
...
これは{\tt https://github.com/herumi/prml/raw/master/prml3.pdf}で求めた線形回帰モデルの予測分布の分散
$$
\sigma_N^2(x)=\frac{1}{\beta}+\trans{\phi(x)}S_N\phi(x)
$$
でノイズを消したもの（$\beta \rightarrow \infty$）$\sigma_a^2$に一致する.
