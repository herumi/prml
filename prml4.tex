...
二乗誤差関数
$$
E_D(W)=\half\tr(\trans{(XW-T)}(XW-T))
$$
を最小化する$W$を求めよう.

\begin{eqnarray*}
\diff{w_{ij}}E_D(W)
 &=& \half\diff{w_{ij}}\sum_{s,t} ((XW-T)_{st})^2\\
...
$$
p(x|C_k)=\frac{1}{(2\pi)^(D/2)}\frac{1}{|A|^{1/2}}\exp(-\half\trans{(x-\mu_k)}A^{-1}(x-\mu_k))
$$
\begin{eqnarray*}
...
\begin{eqnarray*}
 && -\half\sum_{n=1}^N(t_n \log |A|+t_n \trans{(x_n-\mu_1)}A^{-1}(x_n-\mu_1)+(1-t_n) \log |A|+(1-t_n)\trans{(x_n-\mu_2)}A^{-1}(x_n-\mu_2))\\
 &=& -\frac{N}{2}\log |A| - \half \tr(\sum_{n=1}^N(t_n A^{-1}(x_n -\mu_1)\trans{(x_n-\mu_1)}+(1-t_n)A^{-1}(x_n-\mu_2)\trans{(x_n-\mu_2)}))\\
 &=& -\frac{N}{2}\log |A| -\half \tr(A^{-1}(\sum_{n\in C_1}(x_n-\mu_1)\trans{(x_n-\mu_1)}+\sum_{n\in C_2}(x_n-\mu_2)\trans{(x_n-\mu_2)}))\\
 &=& -\frac{N}{2}\log |A| -\half \tr(A^{-1}(N_1 S_1 + N_2 S_2))\\
...
この式を線形回帰モデルに適用してみる.
$$
E(w)=\half\sum(t_n-\trans{w}\phi(x_n))^2, \phi_n=\phi(x_n)
$$
...
\begin{eqnarray*}
\Phi(a) &=& \int_{-\infty}^a = \int_{-\infty}^0 + \int_0^a=\half+\int_0^a \frac{1}{\sqrt{2\pi}}\exp(-\frac{\theta^2}{2})\,d\theta\\
 &=&\half+\int_0^{a/\sqrt{2}}\frac{1}{\sqrt{2\pi}}\exp(-x^2)\sqrt{2}\,dx\\
 &=&\half(1+\frac{2}{\sqrt{\pi}}\int_0^{a/\sqrt{2}} \exp(-x^2)\,dx)\\
 &=&\half\{1+\makeop{erf}(\frac{a}{\sqrt{2}})\}.
\end{eqnarray*}
...
つまり
$$
f(z) \approx f(z_0) \exp(-\half A(z-z_0)^2).
$$
よって正規分布で近似すると
$$
q(z)=(\frac{A}{2\pi})^{1/2} \exp(-\half A(z-z_0)^2).
$$
...
とすると$A>0$（正定値）で
$$
f(z) \approx f(z_0) \exp (-\half\trans{(z-z_0)}A(z-z_0)).
$$
よって
$$
q(z)=\calN(z|z_0,A^{-1})=\sqrt{\frac{|A|}{(2\pi)^M}} \exp (-\half\trans{(z-z_0)}A(z-z_0)).
$$
...
$$
Z=\int f(z)\,dx \approx \int f(z_0) \exp(-\half\trans{(z-z_0)}A(z-z_0))\,dx=f(z_0)\sqrt{\frac{(2\pi)^M}{|A|}}.
$$
...
$$
$$
\log p(D)=\log Z \approx \log f(\theta_{\text{MAP}}) + \log \sqrt{\frac{(2\pi)^M}{|A|}}=\log p(D|\theta_{\text{MAP}})+(\log p(\theta_{\text{MAP}})+\frac{M}{2} \log (2\pi) - \half \log |A|).
$$
括弧内の後ろ三項をOccam係数という.
...
$$
\sigma'(0)=\sigma(a)(1-\sigma(a))\Bigr|_{a=0}=\half(1-\half)=\frac{1}{4}.
$$
$$
\Phi'(\lambda a)=\frac{1}{\sqrt{2\pi}}\lambda \exp(-\half(\lambda a)^2)
$$
...
$$
$$
f(x)=\calN(x|0,1)=\frac{1}{2\pi}\exp(-\half x^2)
$$
とおく.
$$
...
$$
\calN(a|\mu,\sigma^2)=\frac{1}{\sqrt{2\pi}\sigma}\exp(-\frac{1}{2\sigma^2}(\mu+\sigma x-\mu)^2)=\frac{1}{\sigma}\frac{1}{\sqrt{2\pi}}\exp(-\half x^2)=\frac{f(x)}{\sigma}.
$$
よって
$$
...
$$
$$
\diff{\mu}L=\int_{-\infty}^\infty \lambda f(\lambda \mu + \lambda \sigma x)f(x)\,dx=\lambda \int_{-\infty}^\infty \Bigl(\frac{1}{\sqrt{2\pi}}\Bigr)^2\exp(-\half(\lambda\mu+\lambda \sigma x)^2-\half x^2)\,dx.
$$
$\exp()$の内側を$x$について平方完成しよう：
$$
-\half((\lambda\sigma)^2+1)x^2+2\lambda^2\mu \sigma x + \lambda^2 \mu^2)=-\half\Bigl((\sqrt{1+\lambda^2\sigma^2}x+\frac{\lambda^2\mu \sigma}{\sqrt{1+\lambda^2\sigma^2}})^2+\lambda^2 \mu^2-\frac{\lambda^4 \mu^2 \sigma^2}{1+\lambda^2 \sigma^2} \Bigr).
$$
定数項は
$$
...
より
$$
I:=\int_{-\infty}^\infty \exp(-\half(\sqrt{1+\lambda^2\sigma^2}x+\frac{\lambda^2\mu\sigma}{\sqrt{1+\lambda^2\sigma^2}})^2)\,dx=\frac{\sqrt{2\pi}}{\sqrt{1+\lambda^2\sigma^2}}.
$$
よって
\begin{eqnarray*}
\diff{\mu}L &=& \frac{\lambda}{2\pi} \exp(-\half\Bigl(\frac{\lambda^2\mu^2}{1+\lambda^2 \sigma^2}\Bigr)) I\\
 &=& \frac{\lambda}{2\pi} \exp(-\half\Bigl(\frac{\lambda^2\mu^2}{1+\lambda^2 \sigma^2}\Bigr)) \frac{\sqrt{2\pi}}{\sqrt{1+\lambda^2\sigma^2}}\\
...
よって
$$
\log p(w|t)=-\half\trans{(w-m_0)}S_0^{-1}(w-m_0)+\sum_{n=1}^N (t_n \log y_n + (1-t_n)\log (1-y_n)) + \text{const}.
$$
