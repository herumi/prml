...
なぜなら,
$$
\left(\diff{A}\tr(AB)\right)_{ij}=\diff{a_{ij}}\sum_{s,t} a_{st} b_{ts} = b_{ji}.
$$
ここで
...
\begin{eqnarray*}
\diff{w_{ij}}E_D(W)
 &=& \half\diff{w_{ij}}\sum_{s,t} \left((XW-T)_{st}\right)^2\\
 &=& \sum_{s,t} (XW-T)_{st} \diff{w_{ij}}(XW-T)_{st}\\
 &=& \sum_{s,t} (XW-T)_{st} \diff{w_{ij}}\left(\sum_u x_{su}w_{ut}\right)\\
...
$$
\diff{w}J(w)=\left(2 (S_B w)(\trans{w} S_W w) - 2 (\trans{w} S_B w) (S_W w)\right)/(\trans{w}S_W w)^2=0.
$$
よって
$$
...
よって
$$
\diff{w}E=\sum\left(\trans{w}x_n + w_0-t_n\right)x_n=0
$$
を使うと
$$
...
\left(S_W+\frac{N_1 N_2}{N}S_B\right)w&=&N(\bmm_1-\bmm_2)\\
                           &+&\left(N\bmm \trans{\bmm}-N_1\bmm_1 \trans{\bmm_1}-N_2 \bmm_2 \trans{\bmm_2}+\frac{N_1 N_2}{N} (\bmm_1 -\bmm_2)\trans{(\bmm_1-\bmm_2)}\right)w.
\end{eqnarray*}

$()$内が0であることを示す（めんどうなので$\bmm_i$を$m_i$と略する）.
\begin{eqnarray*}
()
&=& \frac{1}{N}\outp{\left(N_1 m_1+N_2 m_2\right)}-N_1 \outp{m_1}-N_2 \outp{m_2}+\frac{N_1 N_2}{N}\left(m_1 \trans{m_2}+m_2 \trans{m_2}\right)\\
&=& \left(\frac{N_1^2}{N}-N_1+\frac{N_1 N_2}{N}\right)\outp{m_1}+\left(\frac{2}{N} N_1 N_2 - \frac{2}{N}N_1 N_2\right)m_1\trans{m_2}+\left(\frac{N_2^2}{N}-N_2+\frac{N_1 N_2}{N}\right)\outp{m_2}.
\end{eqnarray*}
$$
...
\begin{eqnarray*}
 ...
 &=& -\frac{N}{2}\log |A| - \half \tr\left(\sum_{n=1}^N\left(t_n A^{-1}(x_n -\mu_1)\trans{(x_n-\mu_1)}+(1-t_n)A^{-1}(x_n-\mu_2)\trans{(x_n-\mu_2)}\right)\right)\\
 &=& -\frac{N}{2}\log |A| -\half \tr\left(A^{-1}\left(\sum_{n\in C_1}(x_n-\mu_1)\trans{(x_n-\mu_1)}+\sum_{n\in C_2}(x_n-\mu_2)\trans{(x_n-\mu_2)}\right)\right)\\
...
よって
\begin{eqnarray*}
\diff{w}E &=&-\sum\left(t_n (1-y_n)\phi_n + (1-t_n)(-y_n)\phi_n\right)\\
...
$$
\diff{a_k}y_k=\frac{\exp(a_k)\left(\sum \exp(a_j)\right)-\exp(a_k)\exp(a_k)}{\left(\sum \exp(a_j)\right)^2}=y_k-y_k^2.
$$
$k\ne j$として
$$
...
\begin{eqnarray*}
\Phi(a) &=& \int_{-\infty}^a = \int_{-\infty}^0 + \int_0^a=\half+\int_0^a \frac{1}{\sqrt{2\pi}}\exp\left(-\frac{\theta^2}{2}\right)\,d\theta\\
...
$\eta$で微分して
\begin{eqnarray*}
&& \left(\diff{\eta}g(\eta)\right) \int h(t/s)\exp\left(\frac{\eta t}{s}\right)\,dt+g(\eta)\int (t/s) h(t/s) \exp\left(\frac{\eta t}{s}\right)\,dt\\
&=& \left(\diff{\eta}g(\eta)\right)\left(\frac{s}{g(\eta)}\right)+\int t p(t|\eta,s)\,dt\\
...
$$
Z=\int f(z)\,dx \approx \int f(z_0) \exp\left(-\half\trans{(z-z_0)}A(z-z_0)\right)\,dx=f(z_0)\sqrt{\frac{(2\pi)^M}{|A|}}.
$$
...
$$
\diff{\mu}R=f\left(\frac{\lambda \mu}{\sqrt{1+\lambda^2\sigma^2}}\right) \frac{\lambda}{\sqrt{1+\lambda^2\sigma^2}}.
$$
$$
\diff{\mu}L=\int_{-\infty}^\infty \lambda f(\lambda \mu + \lambda \sigma x)f(x)\,dx=\lambda \int_{-\infty}^\infty \Bigl(\frac{1}{\sqrt{2\pi}}\Bigr)^2\exp\left(-\half(\lambda\mu+\lambda \sigma x)^2-\half x^2\right)\,dx.
$$
$\exp()$の内側を$x$について平方完成しよう：
$$
-\half\left((\lambda\sigma)^2+1\right)x^2+2\lambda^2\mu \sigma x + \lambda^2 \mu^2)=-\half\left(\left(\sqrt{1+\lambda^2\sigma^2}x+\frac{\lambda^2\mu \sigma}{\sqrt{1+\lambda^2\sigma^2}}\right)^2+\lambda^2 \mu^2-\frac{\lambda^4 \mu^2 \sigma^2}{1+\lambda^2 \sigma^2} \right).
$$
定数項は
$$
...
よって
\begin{eqnarray*}
\diff{\mu}L &=& \frac{\lambda}{2\pi} \exp\left(-\half\left(\frac{\lambda^2\mu^2}{1+\lambda^2 \sigma^2}\right)\right) I\\
 &=& \frac{\lambda}{2\pi} \exp\left(-\half\Bigl(\frac{\lambda^2\mu^2}{1+\lambda^2 \sigma^2}\Bigr)\right) \frac{\sqrt{2\pi}}{\sqrt{1+\lambda^2\sigma^2}}\\
...
よって
\begin{eqnarray*}
p(C_1|\phi,t) &\approx& \int \left(\int \delta(a-\trans{w}\phi)\sigma(a)q(w)\,da\right) dw\\
