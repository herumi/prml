...
$$
データ集合$D$の点の個数を$N$とし, それぞれが独立であると考えると$H$の各要素は$O(N)$. $B$は$N$や次元の数$M$には依存しないので$M$, $N$を大きくしたとき無視できるとすると
$$
...
$$
これをベイズ情報量基準（Bayesian Information Criterion, BIC）という.

\section{ディラックのデルタ関数}
気持ち的にはデルタ関数は$\RR$から$\RR$への関数で
$$
\delta(x)=
\begin{cases}
1 & (x=0) \\
...
\end{cases}
$$
である. ただ実際にはデルタ関数は積分を通してしか扱われない.

任意の実連続関数$f(x)$に対して
$$
\int_{-\infty}^\infty f(x) \delta(x)\,dx = f(0)
$$
とうい性質を満たす超関数（ここでは定義しない）として解釈する.
...
$$
p(C_1|\phi,t)=\int p(C_1|\phi,w)p(w|t)\,dt \approx \int \sigma(\trans{w}\phi)q(w)\,dw.
$$
デルタ関数の性質から
$$
