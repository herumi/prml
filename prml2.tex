...
全ての$x$について$f(-x)=-f(x)$のとき$f$を奇関数という. \\
$f(-x)=f(x)$のときは偶関数という.
...
3個の行列の積については$\tr(ABC)=\sum_i (ABC)_{ii} = \sum_i \left(\sum_{j,k} a_{ij} b_{jk} c_{ki}\right) = \sum_{i,j,k} a_{ij} b_{jk} c_{ki}$より
$$\tr(ABC)=\tr(BCA)=\tr(CAB).$$
...
$A$を$m$行$n$列の行列とする. $A$に対して$\trans{A} = (a_{ji})$を$A$の\textgt{転置行列}という. これは$n$行$m$列の行列である.
$|\trans{A}|=|A|$, $\trans{(AB)}=\trans{B}\trans{A}$である.

$\overline{A}=(\overline{a_{ij}})$を$A$の\textgt{複素共役行列}, $A^*=\overline{\trans{A}}$を\textgt{随伴行列}という.
$|A^*|=\overline{|A|}$, $(AB)^*=B^* A^*$である.

$a_{ii}$を\textgt{対角成分}という. 対角成分以外の項が0である行列を\textgt{対角行列}といい $\diag(a_0, \ldots, a_n)$と書く.
...
\end{cases}
$$
を\textgt{クロネッカーのδ}といい, $I_n=(\delta_{ij})$を\textgt{$n$次単位行列}という. $I$と略すこともあるし$E$と書くこともある.

$A$を\textgt{$n$次正方行列}とすると$AI_n=I_nA=A$.

$A$が$n$次正方行列で, $|A|\neq0$のとき$A$を\textgt{正則}といい, $AB=BA=I$となる行列$B$が存在する.
$B$を\textgt{逆行列}といい, $A^{-1}$と書く.
...
まず$f(\bm{z})=\exp\left(-(1/2)\quadf{A^{-1}}{z}\right)$を考える.
...
これに$\calN\left(\bm{x}|\bm{\mu}, A\right)$をかけた値を積分するわけだが, 第2項, 第3項は奇関数になるので0.
...
\begin{eqnarray*}
\tr\left(A^{-1}A'\right)&=&-\tr\left(\left(P^{-1}\tri(\lambda_i)^{-1}P\right)P^{-1}P'P^{-1}\tri(\lambda_i)P\right)\\
...
&=&\sum_s \left(\sum_t (A^{-1})_{st} \delta_{it} \delta_{js}\right) = (A^{-1})_{ji}.
\end{eqnarray*}
\end{enumerate}
