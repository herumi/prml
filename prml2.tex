...
$x_i = g(y_1, \ldots, y_n) \mbox{ for $i=1, \ldots, n$}$とすると
...
$$I^2=\int_0^{\pi/2} \int_0^\infty \exp(-r^2)r \,drd\theta=\pi/2 \left[ -\frac{1}{2}\exp(-r^2)\right] _0^\infty=\pi/4.$$
...
つまりガウス分布を
$${\cal N}(x|\mu, \sigma^2)=\frac{1}{\sqrt{2\pi}\sigma}\exp(-\frac{1}{2\sigma^2}(x-\mu)^2)$$
とすると
$$\int_{-\infty}^\infty {\cal N}(x|\mu, \sigma^2) \,dx=1.$$

平均は
$$x {\cal N}(x|\mu, \sigma^2) = (x-\mu) {\cal N}(x|\mu, \sigma^2) + \mu {\cal N}(x|\mu, \sigma^2)$$

とわけると, 第1項は$(x-\mu)$に関して奇関数なので積分すると消えて
$$\EE[x]=\int_{-\infty}^\infty x {\cal N}(x|\mu, \sigma^2) \,dx=\mu.$$

分散は$x^2=(x-\mu)^2+2\mu(x-\mu)+\mu^2$なので積分すると第1項は式(\ref{int_xx})より$\sigma^2$.
第2項は0. 第3項は$\mu^2$.

よって
$$\EE[x^2]=\int_{-\infty}^\infty x^2 {\cal N}(x|\mu, \sigma^2) \,dx=\sigma^2 + \mu^2.$$
...
$$f(\bm{z})
=\exp(-\frac{1}{2}\sum_{i=1}^n \frac{z_i^2}{\lambda_i})
...
よって
$${\cal N}(\bm{x}|\bm{\mu}, A)
  =\frac{1}{\sqrt{2\pi}^n} \frac{1}{\sqrt{|A|}}\exp(-\frac{1}{2}\trans{(\bm{x-\mu})}A^{-1}(\bm{x-\mu}))$$
とすると正規化されている.
これが多変量版のガウス分布である.
...
これに${\cal N}(\bm{x}|\bm{\mu}, A)$をかけた値を積分するわけだが, 第2項, 第3項は奇関数になるので0.
...
対数尤度関数
$$\log p(\bm{X}|\bm{\mu}, A)
=-\frac{Nn}{2}\log(2\pi)-\frac{N}{2}\log|A|-\frac{1}{2}\sum_{i=1}^N \trans{(\bm{x_i-\mu})}A^{-1}(\bm{x_i-\mu})$$
を$A$についての最大化を求めてみる. ここではPRMLでは式変形の途中で対称性を利用しているが,
せっかくなので$A$の対称性を仮定せずに話を進める.

その前にまず$A$を固定したときの$\bm{\mu}$に関する最尤推定の解を求めておこう.
式(\ref{diff_quad})より
$$\diff{\bm{\mu}} \log p(\bm{X}|\bm{\mu}, A)
=\frac{1}{2}\sum_{i=1}^N (A^{-1}+\trans{(A^{-1})})(\bm{x_i-\mu})=\frac{1}{2}(A^{-1}+\trans{(A^{-1})})((\sum_{i=1}^N \bm{x_i}) - N\bm{\mu}).$$
これが$\bm{0}$なので
