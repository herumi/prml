...
つまりガウス分布を
$$N(x|\mu, \sigma^2)=\frac{1}{\sqrt{2\pi}\sigma}\exp(-\frac{1}{2\sigma^2}(x-\mu)^2)$$
とすると
$$\int_{-\infty}^\infty N(x|\mu, \sigma^2) \,dx=1.$$

平均は
$$x N(x|\mu, \sigma^2) = (x-\mu) N(x|\mu, \sigma^2) + \mu N(x|\mu, \sigma^2)$$

とわけると, 第1項は$(x-\mu)$に関して奇関数なので積分すると消えて
$$\EE[x]=\int_{-\infty}^\infty x N(x|\mu, \sigma^2) \,dx=\mu.$$

分散は$x^2=(x-\mu)^2+2\mu(x-\mu)+\mu^2$なので積分すると第1項は式(\ref{int_xx})より$\sigma^2$.
第2項は0. 第3項は$\mu^2$.

よって
$$\EE[x^2]=\int_{-\infty}^\infty x^2 N(x|\mu, \sigma^2) \,dx=\sigma^2 + \mu^2.$$
...
これに$N(\bm{x}|\bm{\mu}, A)$をかけた値を積分するわけだが, 第2項, 第3項は奇関数になるので0.
