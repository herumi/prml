...
3個の行列の積については$\tr(ABC)=\sum_i (ABC)_{ii} = \sum_i (\sum_{j,k} a_{ij} b_{jk} c_{ki}) = \sum_{i,j,k} a_{ij} b_{jk} c_{ki}$より
$$\tr(ABC)=\tr(BCA)=\tr(CAB).$$
...
式(\ref{diff_quad})より
\begin{eqnarray*}
\diff{\bm{\mu}} \log p(\bm{X}|\bm{\mu}, A)&=&\half\sum_{i=1}^N(A^{-1}+\trans{(A^{-1})})(\bm{x_i-\mu})\\
&=&\half(A^{-1}+\trans{(A^{-1})})((\sum_{i=1}^N \bm{x_i}) - N\bm{\mu}).
\end{eqnarray*}
これが$\bm{0}$なので
...
\begin{eqnarray*}
\diff{a_{ij}}\tr(A^{-1}B)&=&\tr((\diff{a_{ij}}A^{-1})B)\\
