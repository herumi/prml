...
= \diff{\mu} \left(-\half\quads{{\Sigma^{-1}}}{{(x-\mu)}}\right)
= \Sigma^{-1}(x-\mu)
$$
より
$$
\diff{\mu} \calN = \calN \cdot \left(\diff{\mu} \log \calN\right) = \calN \cdot \Sigma^{-1}(x-\mu).
$$
...
\diff{\mu_k}F
 ...
 &=& \sum_n \left(\frac{\pi_k \calN_{nk}}{\sum_j \pi_j \calN_{nj}}\right)\diff{\mu_k}\log \calN_{nk} \\
...
\diff{\Sigma_k}F
 ...
 &=&\sum_n \gamma(z_{nk})\left(-\half(\Sigma_k^{-1})
        + \half(\Sigma_k^{-1}\outp{(x_n-\mu_k)}\Sigma_k^{-1})\right) = 0.
\end{eqnarray*}
よって
$$
...
として
$$
\diff{\pi_k}G=\sum_n z_{nk}\frac{1}{\pi_k}+\lambda=\left(\sum_n z_{nk}\right)/\pi_k + \lambda=0.
$$
よって
$$
...
$$
$$
\diff{\mu_k}F=\sum_n\gamma(z_{nk})\left(-\Sigma_k^{-1}(x_n-\mu_k)\right)=\Sigma_k^{-1}\left(\sum_n \gamma(z_{nk})x_n-\left(\sum_n \gamma(z_{nk})\right)\mu_k\right)=0.
$$
よって
$$
\mu_k=\frac{1}{N_k}\sum_n \gamma(z_{nk})x_n.
...
\cov[x]&=&E[\outp{(x-E[x])}]\\
       &=&E[\outp{x}]-\outp{E[x]}\\
       &=&\sum_k \pi_k(\Sigma_k+\outp{\mu_k})-\outp{E[x]}.
\end{eqnarray*}
...
F &=& E[\log p(t,w|\alpha,\beta)]\\
 ...
 &=& \frac{M}{2}\log \left(\frac{\alpha}{2\pi}\right)-\frac{\alpha}{2}E[\inp{w}]
     +\frac{N}{2}\log \left(\frac{\beta}{2\pi}\right)-\frac{\beta}{2}\sum_n E[(t_n-\trans{w}\phi_n)^2].
\end{eqnarray*}
