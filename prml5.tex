...
例えば一つの式の中に$a_j$と$a_k$があって, $j=1$の$a_j$と$k=1$の$a_k$は別のものを指すことがある.
慣れてしまえば読むのは難しくないが, ここではあえて区別してみる.
...
これらの線形和をとって出力ユニット活性を求める. $z_0=1$を定数変数として
$$
...
$$
$y_{nk}=y_k(x_n,w)$とし$n$番目の入力$x_n$に対する目標変数$t_{nk}$で表す.
$t_{nk}=0$または1であり, $\sum_k t_{nk}=1$である.
$$
...
$$
$K$個のクラス分類問題を考える. 同様に$n$番目の入力$x_n$に対する目標変数$t_{nk}$で表す.
...
$$
の形にできる. そして$E(w)$が$w=w^*$の付近で極小となるのは$H(E) > 0$であることがわかる.
