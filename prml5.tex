...
スカラー$x$についての関数$E(x)$の$x=a$におけるテイラー展開の2次の項までによる近似式は
$$
E(x) \approx E(a) + E'(a)(x-a) + \half E''(a) (x-a)^2
$$
であった. これを$n$変数関数$E(x_1, \ldots, x_n)$に拡張すると,
...
\end{eqnarray*}
となる.
$E(x)$が$x=a$の付近で極小となるとすると, そこでの勾配$\nabla E$は0なので
$$
...
\section{ソフト重み共有}
ネットワークに属する重みを等しくすることで複雑さを減らす手法があるがその制約は厳しい.
ここではそれに代わるソフト重み共有という手法を考える.
ソフト重み共有は重みが等しいという制約の代わりに$M$個の2乗ノルムを正則化項として誤差関数に追加することで,
重みが$M$個のグループに分類されるようにする手法である.
...
重みが複数のグループを構成するようにするために正則化項を考える.
2乗ノルムの正規化項はガウス事前分布の負の対数尤度関数とみなせた.
ここでは複数個を考えるため混合ガウス分布を用いる.
$$
...
$$
前節と同様$\pi_k(x)$を$x$に関する事前分布として対応する事後分布を
$$
